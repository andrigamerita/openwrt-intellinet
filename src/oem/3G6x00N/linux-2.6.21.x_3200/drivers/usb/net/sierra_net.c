 ...
 *
 * Copyright (C) 2008, 2009 Paxton Smith, Matthew Safar, Rory Filer
...
/* atomic counter partialy included in MAC address to make sure 2 devices
...
static u64 get_dns(struct usbnet *dev)
{
	...
	retval = cpu_to_be64(((u64)priv->dns1_addr << 32)|(u64)priv->dns1_addr);
...
{
	devdbg(priv->usbnet, "%s %d", __FUNCTION__, ctx_ix);
...
static int sierra_net_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (!is_whitelisted(ifacenum, &data->whitelist) ) {
		devdbg(dev, "Ignoring interface: %d", ifacenum);
		return -ENODEV;
	}
	...
	if ((status = usbnet_get_endpoints(dev, intf)) < 0) {
		deverr(dev, "Error in usbnet_get_endpoints (%d)", status);
		return -ENODEV;
	}
