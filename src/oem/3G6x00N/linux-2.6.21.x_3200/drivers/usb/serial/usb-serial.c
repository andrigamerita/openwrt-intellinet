...
static int maxDSize = 4096; // tommy+10032008
static int maxUSize = 4096; // tommy+10032008
			       ...
			       const struct usb_device_id *id)
{
	...
	for (i = 0; i < num_bulk_in; ++i) {
		...
		buffer_size = (le16_to_cpu(endpoint->wMaxPacketSize) > maxDSize)?le16_to_cpu(endpoint->wMaxPacketSize):maxDSize; // tommy+10032008
	...
	for (i = 0; i < num_bulk_out; ++i) {
		...
		//buffer_size = le16_to_cpu(endpoint->wMaxPacketSize);
		buffer_size = (le16_to_cpu(endpoint->wMaxPacketSize) > maxUSize)?le16_to_cpu(endpoint->wMaxPacketSize):maxUSize; // tommy+10032008
