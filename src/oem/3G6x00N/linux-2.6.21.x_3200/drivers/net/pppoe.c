...
#if defined(_PPPOE_AUTO_PADT_)	// SJC for Logitec
#include <linux/kmod.h>
//Kyle remove by 2008.10.01
//static int	__item_cnt = 0;
#endif
...
#if defined(_PPPOE_AUTO_PADT_)	// SJC for Logitec
static int is_ppp_echo(struct sk_buff *skb)
{
	struct sk_buff	*skb2;
	u_char	code;
//	skb2 = skb_clone(skb, GFP_ATOMIC);
//	skb_pull(skb2, sizeof(struct pppoe_hdr));
	code = skb->data[8];
//	code = skb2->data[2];
//	kfree_skb(skb2);
	return (code == 0x09);	// is echo request?
}

static void sendPADT(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt, u_short sid)
{
	struct sk_buff	*skb2;
	struct pppoe_hdr hdr;
	struct pppoe_hdr *ph;
	//Kyle add by 2008.10.01
	int data_len = skb->len;
	if (is_ppp_echo(skb) == 0)
		return;
	
	/*Kyle add for Debug
	printk("CheckMac-SKB:%02x%02x%02x%02x%02x%02x\n",skb->mac.ethernet->h_dest[0],skb->mac.ethernet->h_dest[1],skb->mac.ethernet->h_dest[2],skb->mac.ethernet->h_dest[3],
			skb->mac.ethernet->h_dest[4],skb->mac.ethernet->h_dest[5],skb->mac.ethernet->h_dest[6]);
	printk("CheckMac-DEV:%02x%02x%02x%02x%02x%02x\n",dev->dev_addr[0],dev->dev_addr[1],dev->dev_addr[2],dev->dev_addr[3],
			dev->dev_addr[4],dev->dev_addr[5],dev->dev_addr[6]);
	printk("SID=%04x Cmp size=%d MAC Cmp: %d\n",sid,6,memcmp(skb->mac.ethernet->h_dest, dev->dev_addr, 6));
	*/
	
	//Kyle Fixed by 2008.10.01
	if (memcmp(eth_hdr(skb)->h_dest, dev->dev_addr,6))	// don't answer any packet which is not for me
		return;
	//End
	//if (memcmp(skb->mac.ethernet->h_dest, dev->dev_addr, sizeof(dev->dev_addr)))	// don't answer any packet which is not for me

	skb2 = dev_alloc_skb(32 + sizeof(struct pppoe_hdr) + dev->hard_header_len);
	if (skb2 == NULL)
		return;
	...
	skb2->dev = dev;
	skb2->protocol = __constant_htons(ETH_P_PPP_DISC);
	ph = (struct pppoe_hdr *) skb_push(skb2, sizeof(struct pppoe_hdr));
	memcpy(ph, &hdr, sizeof(struct pppoe_hdr));


	//Kyle Fixed by 2008.10.01
	dev->hard_header(skb2, dev, ETH_P_PPP_DISC,
			eth_hdr(skb)->h_source, NULL, data_len);
	//end Kyle.
	dev_queue_xmit(skb2);
}
#endif	// SJC for Logitec
