...
int br_handle_frame(struct net_bridge_port *p, struct sk_buff **pskb)
{
	...
	#ifdef _IPV6_BRIDGE_
				if(!ipv6BrEnabled ||  (eth_hdr(skb)->h_proto!= htons(ETH_P_IPV6))) // Block Non IPv6 packet
	#endif
	
	#ifdef _IGMP_L2_PROXY_
				if(!IGMPL2ProxyOpened || !MULTICAST_MAC(skb->mac.ethernet->h_dest) || 
						!(((skb->mac.ethernet->h_proto == htons(ETH_P_IP)) && (skb->nh.iph->protocol == IPPROTO_IGMP)) || (br_fdb_get(br, dest)!=NULL)))
	#endif
				{
