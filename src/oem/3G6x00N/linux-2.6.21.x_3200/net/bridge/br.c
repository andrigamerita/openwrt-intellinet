...
#endif
#ifdef _IPV6_BRIDGE_
struct proc_dir_entry *procipv6=NULL;
int ipv6BrEnabled=0;

static int br_ipv6read_proc(char *page, char **start, off_t off,
                int count, int *eof, void *data)
{
...
static int br_ipv6write_proc(struct file *file, const char *buffer,
                      unsigned long count, void *data)
{
...
struct proc_dir_entry *procBlockLan=NULL;
int blockLanInterface=0;
static int br_blockLanWrite_proc(struct file *file, const char *buffer,
                      unsigned long count, void *data)
{
    unsigned char br_tmp;
    if (count < 2)
    ...
    if (buffer && !copy_from_user(&br_tmp, buffer, 1)) {
    	blockLanInterface = br_tmp - '0';
          return count;
    }
    return -EFAULT;
}
static int br_blockLanRead_proc(char *page, char **start, off_t off,
                int count, int *eof, void *data)
{
    int len;
...
static int __init br_init(void)
{
	...
	if (procigmp) {
		procigmp->read_proc = (read_proc_t*)&bbr_igmpread_proc;
		procigmp->write_proc = (write_proc_t*)&br_igmpwrite_proc;
	}
#endif

#ifdef _LAN_WAN_ACCESS_
	procBlockLan = create_proc_entry("br_blocklan", 0, NULL);
	if (procBlockLan) {
		procBlockLan->read_proc = (read_proc_t*)&br_blockLanRead_proc;
		procBlockLan->write_proc = (write_proc_t*)&br_blockLanWrite_proc;
	}
#endif
		...
		if (procIgmpProxy) {
		    procIgmpProxy->read_proc = (read_proc_t*)&bbr_igmpProxyRead_proc;
