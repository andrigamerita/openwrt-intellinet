...
//#ifdef _planet_
//	#ifdef _6104K_
//		static char_t   websRealm[64] = T("Default: admin/1234");       /* Realm name */
//	#else
//		static char_t   websRealm[64] = T("Default: admin/admin");      /* Realm name */
//	#endif
//#elif defined(_topcom_)
//	static char_t   websRealm[64] = T("Default: admin/admin");      /* Realm name */
//#elif defined(_sitecom_)
//	static char_t   websRealm[64] = T("Default: admin/admin");      /* Realm name */
//#elif defined(_airlive_)
//	static char_t   websRealm[64] = T("Default: airlive/airlive");      /* Realm name */
//#elif defined(_tulip_)
//	static char_t   websRealm[64] = T("Default: admin/admin");      /* Realm name */
//#elif defined(_planet_)
//	static char_t   websRealm[64] = T("Default: admin/admin");      /* Realm name */
//#elif defined(_PCIJP_)
//	static char_t   websRealm[64] = T("Default: admin/password");      /* Realm name */
//#elif defined(_taiPci_)
//	static char_t   websRealm[64] = T("Default: admin/0000");      /* Realm name */
//#elif defined(_ecom_)
//	static char_t   websRealm[64] = T("Default: admin/ecom");      /*Realm name */
//#elif defined(_cnet_)
//	static char_t websRealm[64] = T("Default: root/1234");
//#elif defined(_netcomm_)
//	static char_t   websRealm[64] = T("Default: admin/admin");      /* Realm name */
//#elif defined(_corega_)
//	static char_t websRealm[64] = T("Default: root/");
//#elif defined(_zio_)
//	static char_t websRealm[64] = T("Default: admin/1234");
//#elif defined(_crypto_)
//	static char_t websRealm[64] = T("Default: admin/admin");
//#elif defined(_logitec_)
//	static char_t websRealm[64] = T("Logitec Router"); //admin / 1234
//#else
//		static char_t   websRealm[64] = T("Default: admin/1234");      /* Realm name */
//
//#endif

#if defined(_WEB_LOGIN_ALERT_STRING_)
	static char_t*   websRealm = _WEB_LOGIN_ALERT_STRING_;      /* Realm name */
#else
	static char_t   websRealm[64] = T("Default: admin/1234");      /* Realm name */
#endif
 ...
 *              required data is available for current state.
                websGetInput:
                Return -1 on errors or if the request has been processed,
 *              1 if input read, and 0 to instruct the caller to call again for more input.
                 NOTE: encounter '\n' will return 1 and data >  256, return 0
                 instruct the caller to call again for more input
 ...
 */
                ...
                // printf("%d calls [%d] %s",i,nbytes,text);
                     i++;     
/*              
...
#ifdef _topcom_                                            
                     if ( wp->flags & WEBS_HNAP_HTTP && !(wp->flags & WEBS_POST_REQUEST)) {
                                 
                              printf("getdevice\n");
                              get_device_settings(wp,NULL); 
                              done++;
                         }
#endif                         
 ...
 */
                       ...
                       /*joseph chen, Mar 28 ,2006

                        *  to handle soap request

                         */
#ifdef _topcom_                        
                         if ( wp->flags & WEBS_SOAPACTION) {                     
                        
                              (wp->postData)[wp->lenPostData]='\0';
                               HNAP_msg_handler(wp,0,wp->postData,wp->lenPostData,wp->soapAction);                
 ...
 */                              // printf("NOT a POST_REQUEST then call UrlHandler\n");
 ...
 */
        ...
        //joseph
#ifdef _topcom_        
         if (gstrcmp(wp->url, T("/HNAP1/")) == 0 || gstrcmp(wp->url, T("/HNAP1")) == 0) {

             wp->flags |=  WEBS_HNAP_HTTP;
 ...
 */
#ifdef _topcom_                        
               } else if (gstrcmp(key, T("soapaction")) == 0) {
                       wp->soapAction = bstrdup(B_L, value);
                        wp->flags |= WEBS_SOAPACTION;
 ...
 */
                                ...
                                if (cp) {
                                     ...
                                     // wp->userName = bstrdup(B_L, "admin");
                                     // wp->password = bstrdup(B_L, "1234");
                          ...
                          //      printf("wp->userName=%s,wp->password=%s\n",wp->userName,wp->password);
/*
...
if ( _MEMSIZE_ < 32)
	if ( gstrcmp(wp->url, T("/goform/formSaveConfig")) )  //wp->path also
       system("killapp.sh");
 ...
 */
                if (redirect != NULL) {
                     ...
                     //   printf("websRespons:HTTP/1.1 %d %s\n", code, websErrorMsg(code));
 ...
 */
                if (code == 401 ) {
                        if (!(wp->flags & WEBS_AUTH_DIGEST)) {    // basic authtntication
 ...
 */      //joseph
       ...
       if ((wp->flags & WEBS_HEAD_REQUEST) == 0 && (wp->flags & WEBS_SOAPACTION) == 1 && message && *message) {
                websWrite(wp, T("%s\r\n"), message);
                    printf("%#x\n",wp->flags);
                printf("%s\n",message);
        }
...
void websError(webs_t wp, int code, char_t *fmt, ...)
{
   ...
   /* implicit else... */
        ...
        msg = T("<html><head><title>Document Error: %s</title></head>\r\n\
                <body><h2>OCs: %s</h2>\r\n\
				l鎞. <b>%s</b><br><p</p></body></html>\r\n");
	#else
        msg = T("<html><head><title>Document Error: %s</title></head>\r\n\
                 <body><h2>Access Error: %s</h2>\r\n\
                when trying to obtain <b>%s</b><br><p></p></body></html>\r\n");
	#endif
/*
