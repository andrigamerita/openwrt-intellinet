...
#elif  defined(__lynx__) || defined(__sun__) || defined(_AIX) || defined(__FreeBSD__) || defined(__NTVIS__) || defined(__NetBSD__)
   ...
   if (trans) {
     ...
     if (sinkmode) {
	   ...
	   sockets_err("error from the socket write");
         ...
         /* hashes */
         if (hash==1 || (hash > 1 && (numCalls % hash) == 0)) {
           fprintf(stderr,"#");
         }
     ...
     if (!errno) {
       ...
       if(udp && !no_end) {
         ...
         while (end_count-- > 0) {
           ...
           if (bufsleep && end_count>0) usleep(bufsleep);
         }
       }
       result_summary();
     }
