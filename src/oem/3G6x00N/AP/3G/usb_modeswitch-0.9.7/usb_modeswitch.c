...
static struct option long_options[] =
{
	...
	{"QisdaMode",			required_argument, 0, 'B'}, // tommy add for Qisda Flying Beetle
...
void readConfigFile(const char *configFilename)
{
	...
	ParseParamBool(configFilename, QisdaMode); // tommy
...
int readArguments(int argc, char **argv)
{
	...
	while (1)
	{
		...
		c = getopt_long (argc, argv, "hWQs:v:p:V:P:C:m:M:r:d:H:S:O:B:n:c:R:i:u:a:",
		...
		switch (c)
		{
			...
			case 'B': QisdaMode = (toupper(optarg[0])=='Y' || toupper(optarg[0])=='T'|| optarg[0]=='1'); break; // tommy
			case 'n': break;
...
int main(int argc, char **argv) {
	...
	specialMode = DetachStorageOnly + HuaweiMode + SierraMode + SonyMode + QisdaMode;
	if ( specialMode > 1 ) {
...
int switchQisdaMode () {
	...
	if (show_progress) printf("Trying to send Qisda control message\n");
	buf[0] = 0x05;
	buf[1] = 0x8c;
	buf[2] = 0x04;
	buf[3] = 0x08;
	...
	buf[14] = 0xea;
	buf[15] = 0xbf;
	ret = usb_control_msg(devh, 0x40, 0x04, 00000000, 0, buf, 16, 100);
	//if (ret != 0) {
