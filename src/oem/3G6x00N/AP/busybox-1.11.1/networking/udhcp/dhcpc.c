			...
			case REBINDING:
				if (*message == DHCPACK) {
					...
					timeout = lease_seconds / 2;
					{
						...
						FILE *ptr;
						ptr = fopen("/tmp/dhcptemp","w");
						fprintf(ptr,"IP = %s<br>",inet_ntoa(temp_addr));

						if ( (temp = get_option(&packet, DHCP_SUBNET )) )
						{
							fprintf(ptr,"SUBNET = %d.%d.%d.%d<br>",temp[0],temp[1],temp[2],temp[3]);
						}

						if ( (temp = get_option(&packet, DHCP_ROUTER )) )
						{
							fprintf(ptr,"GATEWAY = %d.%d.%d.%d<br>",temp[0],temp[1],temp[2],temp[3]);
						}

						if ( (temp = get_option(&packet, DHCP_DNS_SERVER )) )
						{
							fprintf(ptr,"Primary DNS = %d.%d.%d.%d<br>",temp[0],temp[1],temp[2],temp[3]);
							if(*(temp-1)>4)
								fprintf(ptr,"Secondary DNS = %d.%d.%d.%d<br>",temp[4],temp[5],temp[6],temp[7]);
							if(*(temp-1)>8)
								fprintf(ptr,"Third DNS = %d.%d.%d.%d<br>",temp[8],temp[9],temp[10],temp[11]);
						}
						fclose(ptr);
					}
					requested_ip = packet.yiaddr;
