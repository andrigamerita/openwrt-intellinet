...
void run_applet_and_exit(const char *name, char **argv)
{
	...
	int applet1 = find_applet_by_name("ash"); // alex add for encrypted script can't run issue //2009.11.27
	if (applet >= 0)
	...
	if (applet < 0) // alex add for encrypted script can't run issue //2009.11.27
